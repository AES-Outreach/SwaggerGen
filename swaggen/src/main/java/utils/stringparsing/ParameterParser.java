...
public class ParameterParser implements StringParser<ParsedParameter> {
    ...
    /**
     * Constructs a parser with the given delimiters. Will parse a String with the
     * format "type{typeDelimiter}name{descriptionDelimiter}description"
     * 
    ...
    public ParsedParameter parse(String parameterString) throws ParseException {
        ...
        // Find the first matching pattern then set the appropriate parsed parameter
        // values.
        if (fullParameterMatcher.matches()) {
            ...
            throw new ParameterParseException(parameterString);
